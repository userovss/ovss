...
class TCL(nn.Module):
    ...
    @torch.no_grad()
    def generate_masks(
        self, image, text_emb, kp_branch_text_emb=None, text_is_token=False, apply_pamr=False,
        kp_w=0.3
    ):
        ...
        """
        if kp_branch_text_emb is None:
            kp_branch_text_emb = text_emb
        if text_is_token:
        ...
        image_feat = clip_image_feats[0]
        clip_image_feats = clip_image_feats[1:]
        mask, simmap = self.masker.forward_seg(image, image_feat, text_emb, self.img_fea_encoder, hard=False)  # [B, N, H', W']
        ...
        if kp_w:
            mask = self.kp_branch(img_emb, kp_branch_text_emb, mask, kp_w=kp_w)
        ##############################################
